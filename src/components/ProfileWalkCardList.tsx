        ...
        >
          {walk.photoUrl ? (
            <img
              src={walk.photoUrl}
              alt={walk.themeTitle}
              className={`${getCoverHeightClass(walk)} w-full rounded-t-[28px] object-cover`}
            />
          ) : (
            <div
              className={`${getGradientClass(walk)} ${getCoverHeightClass(walk)} flex items-end rounded-t-[28px] px-5 py-5 text-white`}
            >
              <div>
          ...
          <div className="space-y-3 px-5 py-4">
            <div>
              ...
              <p className="mt-2 text-sm leading-7 text-slate-600">{buildSummary(walk)}</p>
            </div>
            ...
            <div className="flex flex-wrap gap-2 text-xs text-slate-500">
              ...
              {'likeCount' in walk ? (
                <span className="rounded-full bg-slate-100 px-2.5 py-1">赞 {walk.likeCount || 0}</span>
              ) : null}
            ...
            <div className="flex items-center justify-between pt-1">
              <div className="flex items-center gap-2">
                ...
                />
                <div className="text-sm text-slate-600">{walk.authorNickname || fallbackNickname || '我的记录'}</div>
              </div>
